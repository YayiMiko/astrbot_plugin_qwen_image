...
class OneBotImageResolver:
    ...
    async def _save_segment_image(
        ...
        *,
        ...
        self._logger.info(
            "[qwen] %s image segment index=%s data=%s",
            log_prefix,
            index,
        ...
        for candidate in self._candidate_values(data):
            for action, params in self._actions_for(event, self._image_ids(candidate)):
                ...
                if not isinstance(resolved_data, dict):
                    continue
                self._logger.info(
                    "[qwen] %s media resolved action=%s params=%s keys=%s file=%s url=%s base64=%s",
                    log_prefix,
                    action,
                    params,
        ...
        for reply_id in self.reply_ids_from_raw_event(event):
            try:
                ...
                self._logger.warning(
                    "[qwen] failed to fetch raw reply message %s: %s", reply_id, exc
                )
                continue
            segments = (
