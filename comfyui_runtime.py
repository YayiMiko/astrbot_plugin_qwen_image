            ...
            for output in outputs[: self._int("max_send_images", 1)]:
                try:
                    ...
                    if is_ack_timeout(exc, ActionFailed):
                        self.logger.warning(
                            "[qwen] image generated but platform send ACK timed out; "
                        ...
                        return message
                    self.logger.warning(
                        "[qwen] image generated but sending failed. path=%s error=%s: %s",
                        output,
                        type(exc).__name__,
                        str(exc)[:500],
                    )
