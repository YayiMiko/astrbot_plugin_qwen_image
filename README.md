...
- 每张输入图在工作流内最多取 1 MP 作为参考，小图不放大。**单图编辑**默认使用编码器输出的目标图 latent，保持缩放后输入图的宽高比；**双图/三图**继续默认使用独立 4:3、1 MP latent。单图如需固定比例，可把“单图编辑输出尺寸”设为 `configured`
...
图片来源优先级：**本次消息自带/引用的图 > 已标记的槽位图**。第一张恒为改图目标，后面为参考（换装/融合/风格参考）。单图默认跟随目标图宽高比，双图/三图默认按 4:3、1 MP 独立输出；已有配置的多图尺寸设置不变。输入上传前默认不缩小，但内置图会在条件编码前将每张参考图限制到最多 1 MP。
