...
class ImageInputResolver:
    """Resolve chat images into local workspace files for Qwen tasks."""
    ...
    async def _save_image_component(
        ...
        try:
            ...
            self._logger.warning("[qwen] failed to resolve %s image: %s", label, exc)
            return None
        if not source.exists() or not source.is_file():
            self._logger.warning(
                "[qwen] resolved %s image does not exist: %s", label, source
            )
            return None
        ...
        self._logger.info(
            "[qwen] saved %s image input: %s file=%s url=%s path=%s source=%s",
            label,
            target,
            details.get("file"),
            details.get("url"),
        ...
        *,
        ...
        try:
            async with MediaResolver(
                ...
                if not source.exists() or not source.is_file():
                    self._logger.warning(
                        "[qwen] resolved %s media ref does not exist: %s",
                        label,
                        source,
                    )
                    return None
                ...
                self._logger.info(
                    "[qwen] saved %s image input from media ref: %s ref=%s source=%s",
                    label,
                    target,
            ...
            self._logger.warning(
                "[qwen] failed to resolve %s media ref %s: %s",
                label,
    ...
    def _save_base64_image(
        ...
        *,
        ...
        try:
            ...
            self._logger.warning(
                "[qwen] failed to decode %s base64 image: %s", label, exc
            )
            return None
        if not image_bytes:
            return None
        ...
        self._logger.info(
            "[qwen] saved %s image input from base64: %s bytes=%s",
            label,
            target,
            len(image_bytes),
        )
    ...
    async def event_image_inputs(
        self, event: AstrMessageEvent, limit: int = 3
    ) -> list[str]:
        """Resolve up to `limit` images from quoted and direct chat content.

        The first entry is the edit target; the rest are references.
        ...
        Returns:
            Saved local image paths (possibly empty).
        """
        ...
        if reply_images:
            _remember(await self._save_onebot_reply_image(event))
        for index, image in enumerate(reply_images, start=1):
            if len(saved) >= limit:
                break
            _remember(await self._save_image_component(event, image, "reply", index))
        ...
        if len(saved) < limit:
            for index, data in enumerate(
                ...
                if len(saved) >= limit:
                    break
                _remember(
                    await self._save_raw_message_image(event, data, "message", index)
                )
        self.last_summary = {
            "path": saved[0] if saved else "",
            "source": "multi" if saved else "not_found",
            "count": len(saved),
            "reply_images": len(reply_images),
            "direct_images": len(direct_images),
        }
    ...
    def _content_hash(path: Path) -> str | None:
        """Hash file bytes for duplicate detection.
